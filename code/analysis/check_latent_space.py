...
for role in ROLES_TO_INSPECT:
    ...
    # 全発言とラベルをペアにしておく
    paired_data = list(zip(all_utterances, cluster_labels))

    for i in range(kmeans.n_clusters):
        print(f"\nCluster {i}:")

        # ▼▼▼ [追加修正] 代表発言（Representative Utterance）の表示 ▼▼▼
        # AbstractedWerewolfGameでは、データリストの先頭から順に処理し、
        # 最初に見つかった該当クラスタの発言を代表（固定アクション）として登録しています。
        # ここでも同じロジックで「リスト内で最初に出現する該当クラスタの発言」を探します。
        representative_utterance = next((utt for utt, label in paired_data if label == i), None)
        
        if representative_utterance:
            print(f"  [Representative Utterance (Fixed Action used in CFR)]")
            print(f"  \"{representative_utterance}\"\n")
        ...
        if sample_size > 0:
            print(f"  [Random Samples from Cluster]")
            samples = random.sample(utterances_in_cluster, sample_size)
            for sample in samples:
                # スライス[:100]を削除し、全文を表示するように変更
                print(f"  - \"{sample}\"")
