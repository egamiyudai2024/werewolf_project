...
def format_obs_to_vector(observation):
    ...
    #Environmentから直接渡された変数を使用するCFR用ベクトル
    
    # Secret Action (7次元)
    secret_action_vec = np.zeros(NUM_PLAYERS)
    secret_target = observation.get('my_last_secret_target') # int or None
    if secret_target is not None and isinstance(secret_target, int):
        if 0 <= secret_target < NUM_PLAYERS:
            secret_action_vec[secret_target] = 1

    # Announcement (7次元)
    announcement_vec = np.zeros(NUM_PLAYERS)
    dead_target = observation.get('last_announced_dead') # int or None
    if dead_target is not None and isinstance(dead_target, int):
        if 0 <= dead_target < NUM_PLAYERS:
            announcement_vec[dead_target] = 1

    # Voting Result (49次元)
    voting_result_vec = np.zeros(NUM_PLAYERS * NUM_PLAYERS)
    last_votes = observation.get('last_voting_results', {}) # {voter_id: target_id}
    
    if last_votes:
        for voter_id, target_id in last_votes.items():
            try:
                if target_id is None: continue
                v_id, t_id = int(voter_id), int(target_id)
                if 0 <= v_id < NUM_PLAYERS and 0 <= t_id < NUM_PLAYERS:
                    # Flatten index
                    index = v_id * NUM_PLAYERS + t_id
                    voting_result_vec[index] = 1
...
def get_action_prompt(observation, available_actions):
    ...
    if role == "werewolf":
        ...
        role_instruction = (
            ...
            "Share your results strategically to lead the village, but be careful not to be killed too early. "
            "If you found a Werewolf, convince others to vote them out."
        )
    elif role == "doctor":
        role_instruction = (
            f"IMPORTANT: You are player_{p_id} and the Doctor. Your goal is to protect the Seer or confirmed innocents. "
            "You can also protect yourself to survive. Do not reveal your role too early unless necessary."
        )
    else: # Villager
        role_instruction = (
